...
<!-- PRODUCTS - ĐỔ TỪ DATABASE -->
<div class="products">
<%
    for (Product p : allProducts) {
        // Format giá có dấu phẩy
...
%>
    <div class="product">
        ...
        <% } %>
        <% if (p.getIsBestseller() == 1) { %>
            <span class="badge-hot">🔥 HOT</span>
        <% } %>

        <h3><%= p.getName() %></h3>

        <!-- Hiển thị giá: nếu có discount thì gạch giá gốc -->
        <% if (p.getDiscountPrice() > 0) { %>
            <p class="price-original"><del><%= fmt.format((long)p.getPrice()) %>đ</del></p>
            <p class="price-sale"><%= fmt.format((long)p.getDiscountPrice()) %>đ</p>
        <% } else { %>
...
function nextSlide() { index = (index + 1) % images.length; showSlide(); }
function prevSlide() { index = (index - 1 + images.length) % images.length; showSlide(); }
